  ...
  return (
    <div className="space-y-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Análisis detallado de stock y sugerencias de pedido basadas en consumo.</p>
      </div>

      {/* Tarjetas de resumen */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-gray-500 text-xs font-medium uppercase mb-2">Total Productos</div>
          <div className="text-3xl font-bold text-[#1e3a5f]">{resumen?.total_productos || 0}</div>
        </div>
        <div className="bg-white rounded-lg shadow p-6 border-l-4 border-red-500">
          <div className="text-gray-500 text-xs font-medium uppercase mb-2">Stock Total</div>
          <div className="text-3xl font-bold text-gray-700">{resumen?.stock_total || 0}</div>
        </div>
        <div className="bg-white rounded-lg shadow p-6 border-l-4 border-red-500">
          <div className="text-gray-500 text-xs font-medium uppercase mb-2">Productos Críticos</div>
          <div className="text-3xl font-bold text-red-600">{resumen?.criticos || 0}</div>
        </div>
        <div className="bg-white rounded-lg shadow p-6 border-l-4 border-yellow-500">
          <div className="text-gray-500 text-xs font-medium uppercase mb-2">En Alerta</div>
          ...
          {productosCriticos.length > 0 && (
            <div className="bg-white rounded-lg shadow overflow-hidden">
              <div className="px-6 py-4 bg-red-50 border-b border-red-200">
                <h3 className="text-sm font-semibold text-red-700 uppercase tracking-wider">Productos con Stock Crítico</h3>
              </div>
                ...
                <tbody className="divide-y divide-gray-200">
                  {productosCriticos.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td className="px-6 py-4 text-sm text-red-700 font-bold">{p.stock_actual} {p.um}</td>
                      <td className="px-6 py-4 text-sm text-gray-500">{p.consumo_diario}</td>
          ...
          {productosAlerta.length > 0 && (
            <div className="bg-white rounded-lg shadow overflow-hidden">
              <div className="px-6 py-4 bg-yellow-50 border-b border-yellow-200">
                <h3 className="text-sm font-semibold text-yellow-700 uppercase tracking-wider">Productos en Alerta</h3>
              </div>
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                ...
                <tbody className="divide-y divide-gray-200">
                  {productosAlerta.map((p) => (
                    <tr key={p.id}>
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">{p.nombre}</td>
                      <td className="px-6 py-4 text-sm text-yellow-700 font-semibold">{p.stock_actual} {p.um}</td>
